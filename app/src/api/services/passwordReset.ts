 ...
 * @param siteOrigin the origin of the website, without a path element - e.g. https://beta.colouring.Indonesia
 */
...
function getPasswordResetEmail(email: string, token: string, siteOrigin: string): nodemailer.SendMailOptions {
    ...
    Someone has requested a password reset for the Colouring Indonesia account associated with this email address.
    ...
    return {
        text: messageBody,
        subject: 'Reset your Colouring Indonesia password',
        to: email,
        from: 'no-reply@colouring.Indonesia'
    };
}
